import type {
    CedarBoardStock,
    CedarPlanterBuildPlan,
    CedarPlanterDimensions,
    CedarPlanterEstimate,
} from '../../types/planner';
...
const calculateBottomBoards = (width: number, depth: number, boardLength: number) => {
    ...
    return {
        boardCount: Math.ceil(bottomPieces / piecesPerBoard),
        bottomPieces,
    };
};

const calculateLegBoards = () => 2;

const calculateSupportBoards = (longestSide: number) => (longestSide <= 36 ? 1 : 2);
...
export const calculateCedarPlanterEstimate = ({
    ...
    const bottomBoardCount = calculateBottomBoards(width, depth, boardStock.boardLength).boardCount;
    const legBoardCount = calculateLegBoards();
...
export const calculateCedarPlanterBuildPlan = ({
                                                   width,
                                                   depth,
                                                   planterHeight,
                                                   legHeight,
                                               }: CedarPlanterDimensions & { legHeight: number }): CedarPlanterBuildPlan => {
    const estimate = calculateCedarPlanterEstimate({
        width,
        depth,
        planterHeight,
    });

    const { rowsPerFace } = calculateHorizontalSideBoards(
        width,
        depth,
        planterHeight,
        estimate.boardStock.boardLength,
    );

    const { bottomPieces } = calculateBottomBoards(width, depth, estimate.boardStock.boardLength);

    return {
        ...estimate,
        rowsPerFace,
        bottomSlatCount: bottomPieces,
        legHeight,
        totalBuildHeight: planterHeight + legHeight,
        cutList: [
            {
                label: 'Long side boards',
                quantity: rowsPerFace * 2,
                length: width,
            },
            {
                label: 'Short side boards',
                quantity: rowsPerFace * 2,
                length: depth,
            },
            {
                label: 'Bottom slats',
                quantity: bottomPieces,
                length: depth,
            },
        ],
    };
